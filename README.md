--// ============================================================================
--// RYU HUB PREMIUM + - GPO EDITION (ANTI-CHEAT BYPASS & MICRO-TWEEN ENGINE)
--// ============================================================================
...
local Title = Instance.new("TextLabel", Topbar); Title.Size = UDim2.new(0, 350, 1, 0); Title.Position = UDim2.new(0, 20, 0, 0); Title.BackgroundTransparency = 1; Title.Text = "RYUHUB PREMIUM +"; Title.Font = Enum.Font.GothamBlack; Title.TextSize = 20; Title.TextColor3 = Theme.Text; Title.TextXAlignment = Enum.TextXAlignment.Left
...
--// GPO ANTI-CHEAT BYPASS & REMOTE MOTION ENGINE
local Events = ReplicatedStorage:FindFirstChild("Events")
local MotionRemotes = {
Geppo = Events and Events:FindFirstChild("geppo"),
Soru = Events and Events:FindFirstChild("soru"),
Sprint = Events and Events:FindFirstChild("sprint"),
Climb = Events and Events:FindFirstChild("climb")
}

local function RequestLegitMove(actionType, ...)
pcall(function()
if MotionRemotes[actionType] then
if MotionRemotes[actionType]:IsA("RemoteEvent") then
MotionRemotes[actionType]:FireServer(...)
elseif MotionRemotes[actionType]:IsA("RemoteFunction") then
MotionRemotes[actionType]:InvokeServer(...)
end
end
end)
end

--// TP-CHECK RESISTENTER MICRO-TWEEN
local function GPOSafeTween(targetCFrame, customSpeed)
local char = LocalPlayer.Character
local root = char and char:FindFirstChild("HumanoidRootPart")
if not root then return end
...
while tick() - startTime < timeToTake do
    ...
    if tick() - lastRemotePing > 0.2 then
        lastRemotePing = tick()
        RequestLegitMove("Sprint", "rbxassetid://15382065457")
    end
...
CreateSlider(SecFarmAdvanced, "Movement Speed (Tween)", 10, 150, RyuConfig.TweenSpeed, function(val)
...
CreateSlider(SecIslandTP, "Travel Speed", 10, 150, RyuConfig.IslandSpeed, function(val)
...
task.spawn(function()
    ...
    for _, v in pairs(Workspace:GetDescendants()) do
        if v.Name == "Robo" and v:IsA("Model") and v:FindFirstChild("HumanoidRootPart") then
            ...
            if not isStartIslandRobo then
                if distToTarget < closestDist then closestDist = distToTarget; closestRobo = v end
            end
        end
    end
    ...
    local function SpiderLerp(tPos, currentSpeed)
        ...
        while elapsedTime < t do
            ...
            local targetY = y1
            if forwardHit then targetY = math.max(y1, y2)
            else
            ...
            if finalY > currentY + 3 then 
                ...
                if tick() - lastClimbFire > 0.3 then
                    lastClimbFire = tick()
                    task.spawn(function() RequestLegitMove("Climb", true) end)
                end
                ...
                yVelocity = climbRate
                if isWallBlocking then addTime = 0 
                elseif finalY - currentY > 5 then addTime = dt * 0.3 end
            else
                if isClimbing then
                    isClimbing = false
                    task.spawn(function() RequestLegitMove("Climb", false) end)
            ...
            if (lookPos - finalPos).Magnitude > 0.1 then root.CFrame = CFrame.lookAt(finalPos, lookPos)
            else root.CFrame = CFrame.new(finalPos) end
            ...
            if (root.Position - finalPos).Magnitude > 15 then break end
        end
        
        if hum then hum:Move(Vector3.new(0,0,0), false) end
        if isClimbing then task.spawn(function() RequestLegitMove("Climb", false) end) end
...
if inter then
if inter:IsA("RemoteEvent") then inter:FireServer("Geppo")
elseif inter:IsA("RemoteFunction") then inter:InvokeServer("Geppo") end
end
end)
end)
...
local function PerformGunAttack()
pcall(function()
local char = LocalPlayer.Character
local tool = char and char:FindFirstChildOfClass("Tool")
if tool then tool:Activate() end
...
if v:IsA("BasePart") and v.Name ~= "HumanoidRootPart" and v.CanCollide then v.CanCollide = false end
end
end
end
end)
...
if val and val ~= "" and val ~= "None" then state = "Active" end
end
    ...
    if pg then
        for _, v in pairs(pg:GetDescendants()) do
            if v:IsA("TextLabel") and v.Visible then
                ...
                if txt:find("completed") then return "Completed" end
                if v.AbsolutePosition.X < 500 and v.AbsolutePosition.Y < 500 then
                    if txt:match("%d+/%d+") or txt:match("%d+%s*/%s*%d+") then state = "Active" end
                end
            end
        end
    end
...
local root = char and char:FindFirstChild("HumanoidRootPart")
if root then
GPOSafeTween(npcPos * CFrame.new(0, 0, 3.5))
...
if GetQuestStatus() ~= "Active" then FetchQuest() end
end
end
end)
...
for i = 1, 5 do
pcall(function() ReplicatedStorage:WaitForChild("Events"):WaitForChild("stats"):FireServer(statName, nil, 1) end)
end
end
...
```
    ...
    if RyuConfig.TargetNPC and RyuConfig.TargetNPC ~= "" then
        local qState = GetQuestStatus()
        if qState == "Completed" or qState == "None" then
            FetchQuest()
            continue
        end
    end
    ...
    if RyuConfig.TargetMob and RyuConfig.TargetMob ~= "" then
        ...
        if targetMob then
            ...
            while RyuConfig.AutoFarm and mHum and mHum.Health > 0 do
                ...
                if GetQuestStatus() == "Completed" then break end
                ...
                local isGun = RyuConfig.TargetWeapon:lower():find("gun") or RyuConfig.TargetWeapon:lower():find("rifle")
                ...
                if isGun then
                    ...
                    if distToPos > 5 then
                        GPOSafeTween(CFrame.lookAt(attackPos, mRoot.Position))
                    end
