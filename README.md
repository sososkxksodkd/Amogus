...
function CreateLabel(Name, Text)
	if lastMessage == Name..Text then
		...
		-- [MOBILE FIX] TextBox statt TextLabel für natives Markieren auf dem Handy
		...
		-- Breite etwas reduzieren, um Platz für den Copy-Button zu machen
		tl.Size = UDim2.new(0, 260, 0, 84) 
		...
		tl.Size = UDim2.new(0, 260, 0, tl.TextBounds.Y)
		
		-- [MOBILE FIX] Wichtige Eigenschaften für Mobile Copy & Paste
		tl.ClearTextOnFocus = false
		tl.TextEditable = false
		
		-- [MOBILE FIX] Dedizierter Copy Button
		local copyBtn = Instance.new("TextButton")
		copyBtn.Parent = tl
		copyBtn.Size = UDim2.new(0, 50, 0, math.max(20, tl.TextBounds.Y))
		...
		copyBtn.MouseButton1Click:Connect(function()
		    -- Nutzt den in IY definierten everyClipboard fallback[span_4](start_span)[span_4](end_span)
			if everyClipboard then
				pcall(function() everyClipboard(Text) end)
				copyBtn.Text = "COPIED"
		...
		local totalHeight = math.max(tl.TextBounds.Y, 25)
		scroll_2.CanvasSize = UDim2.new(0, 0, 0, alls + totalHeight)
		scroll_2.CanvasPosition = Vector2.new(0, scroll_2.CanvasPosition.Y + totalHeight)
...
CMDs[#CMDs + 1] = {NAME = 'inviteprompt', DESC = 'Prompts the user of your choice that they had invited you. Requires a rejoin to function.'}
...
addcmd("rejoin", {"rj"}, function(args, speaker)
    ...
    if parseBoolean(args[1], false) == true then
        if queueteleport then
            ...
            notify("Incompatible Exploit", "Your exploit does not support this argument (missing queue_on_teleport)")
        end
    end
...
addcmd('allowrejoin',{'allowrj'},function(args, speaker)
	...
	notify("Client AntiTP", `Scripts now may{allow_rj and "" or " not"} make you rejoin the server`)
end)
