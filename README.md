--// ==========================================
--// IMPEL DOWN SCRIPT (ULTIMATE PREMIUM UI WITH AUTO-SAVE & VERA DIAGONAL LOCK)
--// ==========================================
...
task.spawn(function()
    while true do
        ...
        -- SCHRITT 2: Finde und Töte Vera (Diagonal-Behind Farm Logic)
        if _G.ImpelState == "Init" then
            ...
            if vera then
                ...
                if vHum and vRoot and vHum.Health > 0 then
                    ...
                    -- Diagonal hinter sie: -LookVector für hinten, +Y für Höhe (4 Studs zurück, 4 Studs hoch)
                    local attackPos = vRoot.Position - (vRoot.CFrame.LookVector * 4) + Vector3.new(0, 4, 0)
                    local targetCFrame = CFrame.lookAt(attackPos, vRoot.Position)

                    local dist = (root.Position - attackPos).Magnitude
                    if dist > 15 or timeSinceLastHit > 2 then
                        ToggleHover(false)
